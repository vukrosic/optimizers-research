...
class MoEModelConfig:
    ...
    use_unified_optimizer: bool = True  # Use SingleDeviceMuonWithAuxAdam instead of separate optimizers

    def __post_init__(self):
        self.d_k = self.d_model // self.n_heads
        assert self.d_model % self.n_heads == 0, "d_model must be divisible by n_heads"

# Muon optimizer is now imported from the Muon module
...
def setup_muon_optimizer(model: nn.Module, config: MoEModelConfig):
    """Setup Muon optimizer with hybrid approach using official Muon implementation"""
    muon_params = []
    adamw_params = []
    ...
    # Use official SingleDeviceMuon implementation
    muon_optimizer = SingleDeviceMuon(muon_params, lr=config.muon_lr, momentum=0.95, weight_decay=config.weight_decay)
    adamw_optimizer = torch.optim.AdamW(adamw_params, lr=config.muon_lr*0.1, weight_decay=config.weight_decay)

    return [muon_optimizer, adamw_optimizer]

def setup_unified_muon_optimizer(model: nn.Module, config: MoEModelConfig):
    """Alternative setup using SingleDeviceMuonWithAuxAdam for unified optimization"""
    muon_params = []
    adamw_params = []
    ...
    # Create parameter groups for the unified optimizer
    param_groups = [
        dict(params=muon_params, use_muon=True, lr=config.muon_lr, momentum=0.95, weight_decay=config.weight_decay),
        dict(params=adamw_params, use_muon=False, lr=config.muon_lr*0.1, betas=(0.9, 0.95), weight_decay=config.weight_decay)
    ]
    
    # Use unified optimizer
    unified_optimizer = SingleDeviceMuonWithAuxAdam(param_groups)
...
def train_moe_model(config: MoEModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    ...
    # Setup optimizers
    if config.use_unified_optimizer:
        print("🔧 Using unified Muon optimizer with auxiliary Adam")
        optimizers = setup_unified_muon_optimizer(model, config)
    else:
        print("🔧 Using separate Muon and AdamW optimizers")
